...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        tmp = diff_function(typed_word, word, min(
            limit, min_limit))  # 优化，只有比 min_limit 还小才有效
        if tmp <= limit:
            if tmp < min_limit:
                min_limit = tmp
                result = word
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:
        ...
        if limit == 0:
            return 1e9
        return 1 + min(minimum_mewtations(typed[1:], source[1:], limit - 1), minimum_mewtations(typed, source[1:], limit - 1), minimum_mewtations(typed[1:], source, limit - 1))
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    d = {"id": user_id, "progress": count / len(source)}
    upload(d)
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(1, len(timestamps_per_player[i])):
            times[i].append(timestamps_per_player[i][j] -
                            timestamps_per_player[i][j-1])
    return words, times
    # END PROBLEM 9
...
def fastest_words(match_object):
    ...
    [4, 1, 6]
    """
    player_indices = range(len(get_all_times(match_object))
                           )  # contains an *index* for each player
    # contains an *index* for each word
    word_indices = range(len(get_all_words(match_object)))
    # BEGIN PROBLEM 11
    result = [[] for _ in player_indices]
    for i in word_indices:
        min, time = 0, get_time(match_object, 0, i)
        for j in player_indices:
            if get_time(match_object, j, i) < time:
                min, time = j, get_time(match_object, j, i)
        result[min].append(get_word(match_object, i))
    return result
