...
class Player:
    ...
    def debate(self, other):
        a = max(0.1, self.popularity / (self.popularity + other.popularity))
        b = self.random_func()
        if b < a:
            self.popularity += 50
        else:
...
def store_digits(n):
    ...
    def split_number(n):
        ...
        while n // (10 * power) > 0:
            power *= 10
        highest_digit = n // power
        remaining_number = n % power
        return highest_digit, remaining_number
    a, b = split_number(n)
...
def crispr_gene_insertion(lnk_of_genes, insert):
    ...
    while current_gene is not Link.empty:
        gene = current_gene.first
        count = index + 1
        current_codon = gene

        while current_codon is not Link.empty:
            if current_codon.first == "AUG":
                # Insert the codon 'count' times after the first 'AUG'
                for _ in range(count):
                    current_codon.rest = Link(insert, current_codon.rest)
                    current_codon = current_codon.rest
                break  # Stop after the first "AUG"
            current_codon = current_codon.rest
