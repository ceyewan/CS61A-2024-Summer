...
def move_stack(n, start, end):
    ...
    def helper(n, start, end, help):
        if n == 0:
            return
        helper(n - 1, start, help, end)
        print_move(start, end)
        helper(n - 1, help, end, start)

    helper(n, start, end, help)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda f: (lambda x: x(x))(lambda y: f(lambda *args: y(y)(*args))))(lambda self: lambda n: 1 if n == 1 else mul(n, self(sub(n, 1))))
